...
const multiply = (a, b, func) => func(a * b);
...
const each = (arr, func) =>
  arr.forEach((name) => func(name, arr.indexOf(name)));
...
const startWithLetterA = (arr) =>
  arr.filter((word) => word[0].toLowerCase() === "a");
...
const formalGreeting = (arr) => arr.map((name) => `Hello, ${name}`);
...
const findProgrammer = (arr) =>
  arr.filter((person) => person["job"] === "programmer").pop();
...
const productOfArray = (arr) => arr.reduce((total, num) => total * num);
